...
class Product(models.Model):
    ...
    def add_comment(self, comment, author, slug):
        ...
        if not Comment.objects.filter(author=author, slug=slug).exists():
            new_item, _ = Comment.objects.get_or_create(
                comments=comment, author=author, slug=slug
            )
...
class Image(models.Model):
    ...
    product = models.ForeignKey(
        Product, default=None, related_name="images", on_delete=models.PROTECT
    )
...
class Cart(models.Model):
    ...
    def add_to_cart(self, product_slug):
        ...
        new_item, _ = CartItem.objects.get_or_create(
            product=product, item_total=product.price
        )
...
class Order(models.Model):
    ...
    status = models.CharField(
        max_length=200, choices=ORDER_STATUS_CHOICES, blank=True, null=True
    )
...
class Messages(models.Model):
    ...
    pub_date = models.DateTimeField(
        verbose_name=("Дата сообщения"), default=timezone.now
    )
    admin = models.BooleanField(default=False)
...
class Chat(models.Model):
    ...
    def send_message(self, member, message, admin=False):
        ...
        new_message = Messages.objects.create(
            message=message, member=member, admin=admin
        )
